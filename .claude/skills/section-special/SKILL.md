...
5. 按 [共同辨析合同](../tutorial-authoring/references/comparison-and-visuals.md) 判断同名跨领域方法或替代方案是否会造成误用。只有构念、尺度、采样或解释差异会影响后文时，才在正文前段完成定位。

## 跨领域合同

先说明方法在原领域解决什么问题，再判断目标领域是否具有兼容的构念、测量尺度、采样过程和假设。名称相同不代表 estimand、输入或解释相同；比较时同时给出诊断、验证和停止条件。无法确认可迁移性时，明确标为待验证方案，不写成成熟做法。
