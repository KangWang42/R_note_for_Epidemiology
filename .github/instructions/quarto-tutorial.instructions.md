...
- **无图片的教程**：
  - 首选：使用 SVG 绘制简单封面（推荐）
  - 备选：使用 `figure/default-cover.png` 或分类默认封面
  - SVG 封面应包含：清晰的 Logo/文字，统一的背景色，尺寸建议 800x500
...
1. **源头准确**：教程内容（函数参数、用法说明）必须基于最新的官方文档（CRAN manual, pkgdown 网站, GitHub README），严禁只依赖二手博客或 AI 幻觉。
2. **拒绝浅尝辄止**：不要只翻译官方的第一个简单例子。要深入阅读文档，挖掘真正解决用户痛点的参数和高级用法。
3. **全面系统**：教程应致力于成为该主题的中文“完全指南”，覆盖安装、核心功能、进阶技巧和常见坑点。
...
为了保证教程的全面性、科学性和实用性，所有新教程（尤其是介绍 R 包的教程）应参考以下结构：

```markdown
---
title: "PackageName - 一句话定位"
date: "YYYY-MM-DD"
categories: [R包, 分类, 关键词]
image: "figure/cover.png"
---
...
> 用一段话简要介绍该包的核心功能、解决了什么痛点以及适用场景。

- **核心功能**：列出 3-5 个最重要的功能点
- **官方链接**：[CRAN](url) | [GitHub](url) | [文档](url)

### 为什么选择它？

对比同类包的优势（如：比 apply 更清晰、比 base plot 更美观）。

## 🛠 安装与加载

提供 CRAN 和 GitHub 两种安装方式。
...
结合实际场景（使用内置数据集如 iris, mtcars, penguins），演示最常用的功能。
...
构建一个完整的分析流程（从数据清洗到结果展示），串联上述功能点，体现真实工作流。
...
| 函数 | 功能描述 | 典型参数 |
|------|----------|----------|
| `func1()` | 功能描述 | `arg1 = TRUE` |
| `func2()` | 功能描述 | `method = "lm"` |
