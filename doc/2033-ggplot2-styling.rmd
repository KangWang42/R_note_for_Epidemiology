---
title: "ggplot2 样式设置完全指南"
date: "2026-01-14"
categories: [可视化教程, ggplot2, 样式, 进阶]
image: "images/ggplot2-styling-cover.png"
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(
    echo = TRUE,
    warning = FALSE,
    message = FALSE,
    fig.width = 9,
    fig.height = 6
)
```

## 前言

本教程专注于 ggplot2 **图形样式的精细控制**，不讲绘制什么图，而是讲如何把图做得更专业、更美观。涵盖：

- 🎯 **坐标轴**：刻度、标签、范围、变换
- 📊 **图例**：位置、样式、标题、顺序
- 📝 **标题与文本**：标题、副标题、脚注、注释
- 🎨 **美学映射**：颜色、大小、形状的精细控制
- 📐 **分面**：多面板布局与样式
- 🖼️ **主题**：全面的主题定制

```{r}
library(ggplot2)
library(dplyr)
library(scales)

# 创建示例数据
set.seed(42)
demo_data <- data.frame(
    group = rep(c("A组", "B组", "C组", "D组"), each = 50),
    x = c(rnorm(50, 10, 2), rnorm(50, 15, 3), rnorm(50, 20, 2.5), rnorm(50, 25, 4)),
    y = c(rnorm(50, 100, 15), rnorm(50, 150, 20), rnorm(50, 200, 25), rnorm(50, 250, 30)),
    size = runif(200, 1, 10),
    category = sample(c("类型1", "类型2", "类型3"), 200, replace = TRUE)
)

# 基础图形（后续会不断完善）
p_base <- ggplot(demo_data, aes(x = x, y = y, color = group)) +
    geom_point(size = 3, alpha = 0.7)
p_base
```

---

## 第一部分：坐标轴详细设置

坐标轴是图形最重要的组成部分之一，包括刻度线、刻度标签、轴标签、轴线等。

### 坐标轴范围与限制

```{r}
# 方法1：xlim/ylim（会删除范围外的数据）
p_base +
    xlim(5, 30) +
    ylim(50, 300)
```

```{r}
# 方法2：coord_cartesian（只是"放大"，不删除数据）
# 推荐用于放大局部区域
p_base +
    coord_cartesian(xlim = c(10, 25), ylim = c(100, 250))
```

```{r}
# 方法3：scale_*_continuous 设置（最精细的控制）
p_base +
    scale_x_continuous(limits = c(5, 35), expand = c(0, 0)) +
    scale_y_continuous(limits = c(0, 350), expand = c(0, 0))
```

### 坐标轴刻度控制

```{r}
# 设置主刻度和次刻度
p_base +
    scale_x_continuous(
        breaks = seq(5, 35, by = 5), # 主刻度位置
        minor_breaks = seq(5, 35, by = 1), # 次刻度位置
        limits = c(5, 35)
    ) +
    scale_y_continuous(
        breaks = seq(0, 350, by = 50),
        minor_breaks = seq(0, 350, by = 25),
        limits = c(0, 350)
    )
```

```{r}
# 自定义刻度标签
p_base +
    scale_x_continuous(
        breaks = c(10, 15, 20, 25, 30),
        labels = c("低", "中低", "中", "中高", "高") # 自定义标签
    ) +
    scale_y_continuous(
        breaks = seq(100, 300, by = 50),
        labels = function(x) paste0(x, " 单位") # 使用函数
    )
```

```{r}
# 使用 scales 包格式化
p_base +
    scale_x_continuous(labels = scales::label_number(suffix = " L")) +
    scale_y_continuous(labels = scales::label_comma()) # 千分位逗号
```

### 坐标轴标签样式

```{r}
p_base +
    labs(x = "X 变量 (单位)", y = "Y 变量 (单位)") +
    theme(
        # X 轴标签
        axis.title.x = element_text(
            size = 14, # 字号
            face = "bold", # 粗体
            color = "#1f2937", # 颜色
            margin = margin(t = 10) # 上边距
        ),
        # Y 轴标签
        axis.title.y = element_text(
            size = 14,
            face = "bold",
            color = "#1f2937",
            margin = margin(r = 10),
            angle = 90 # 旋转角度（默认90）
        )
    )
```

### 刻度标签样式

```{r}
p_base +
    theme(
        # X 轴刻度标签
        axis.text.x = element_text(
            size = 11,
            color = "#374151",
            angle = 45, # 旋转角度
            hjust = 1, # 水平对齐
            vjust = 1 # 垂直对齐
        ),
        # Y 轴刻度标签
        axis.text.y = element_text(
            size = 11,
            color = "#374151"
        )
    )
```

### 坐标轴线与刻度线

```{r}
p_base +
    theme(
        # 坐标轴线
        axis.line = element_line(color = "#1f2937", linewidth = 0.8),
        # 刻度线
        axis.ticks = element_line(color = "#1f2937", linewidth = 0.5),
        # 刻度线长度
        axis.ticks.length = unit(0.25, "cm"),
        # 去掉面板边框（配合轴线使用）
        panel.border = element_blank()
    )
```

### 对数坐标轴

```{r}
# 创建适合对数变换的数据
log_data <- data.frame(
    x = c(1, 10, 100, 1000, 10000),
    y = c(2, 20, 200, 2000, 20000)
)

ggplot(log_data, aes(x, y)) +
    geom_point(size = 4, color = "#4f46e5") +
    geom_line(color = "#4f46e5") +
    scale_x_log10(
        breaks = scales::trans_breaks("log10", function(x) 10^x),
        labels = scales::trans_format("log10", scales::math_format(10^.x))
    ) +
    scale_y_log10(
        breaks = scales::trans_breaks("log10", function(x) 10^x),
        labels = scales::trans_format("log10", scales::math_format(10^.x))
    ) +
    annotation_logticks(sides = "bl") + # 在左下添加对数刻度
    theme_bw()
```

### 翻转与极坐标

```{r}
# 翻转坐标轴（横向柱状图）
ggplot(demo_data, aes(x = group, y = y)) +
    geom_boxplot(fill = "#4f46e5", alpha = 0.7) +
    coord_flip() +
    theme_minimal()
```

```{r}
# 极坐标（饼图/雷达图基础）
pie_data <- data.frame(
    category = c("A", "B", "C", "D"),
    value = c(25, 35, 20, 20)
)

ggplot(pie_data, aes(x = "", y = value, fill = category)) +
    geom_bar(stat = "identity", width = 1) +
    coord_polar("y") +
    theme_void() +
    scale_fill_brewer(palette = "Set2")
```

---

## 第二部分：图例详细设置

图例是解读图形的关键，需要精心设计。

### 图例位置

```{r}
# 预设位置
p_base + theme(legend.position = "top")
```

```{r}
p_base + theme(legend.position = "bottom")
```

```{r}
p_base + theme(legend.position = "left")
```

```{r}
# 精确位置（坐标 0-1）
p_base +
    theme(
        legend.position = c(0.85, 0.2), # 右下角内部
        legend.background = element_rect(fill = "white", color = "gray80")
    )
```

```{r}
# 隐藏图例
p_base + theme(legend.position = "none")
```

### 图例标题与标签

```{r}
p_base +
    labs(color = "实验分组") + # 修改图例标题
    scale_color_discrete(
        labels = c("对照组", "低剂量", "中剂量", "高剂量") # 修改标签
    )
```

```{r}
# 去掉图例标题
p_base +
    theme(legend.title = element_blank())
```

### 图例样式定制

```{r}
p_base +
    labs(color = "分组") +
    theme(
        # 图例标题
        legend.title = element_text(
            size = 12, face = "bold", color = "#1f2937"
        ),
        # 图例标签
        legend.text = element_text(
            size = 10, color = "#374151"
        ),
        # 图例背景
        legend.background = element_rect(
            fill = "#f9fafb",
            color = "#e5e7eb",
            linewidth = 0.5
        ),
        # 图例键（图标）背景
        legend.key = element_rect(fill = "white"),
        # 图例键大小
        legend.key.size = unit(1, "cm"),
        # 图例内边距
        legend.margin = margin(5, 10, 5, 10)
    )
```

### 图例布局

```{r}
# 水平排列
p_base +
    theme(legend.position = "bottom") +
    guides(color = guide_legend(nrow = 1)) # 单行
```

```{r}
# 自定义列数
p_base +
    theme(legend.position = "bottom") +
    guides(color = guide_legend(ncol = 2)) # 两列
```

### 图例顺序与反转

```{r}
# 反转图例顺序
p_base +
    guides(color = guide_legend(reverse = TRUE))
```

```{r}
# 自定义顺序
p_base +
    scale_color_discrete(limits = c("D组", "C组", "B组", "A组"))
```

### 多个图例的控制

```{r}
# 创建有多个映射的图
p_multi <- ggplot(demo_data, aes(x = x, y = y, color = group, size = size, shape = category)) +
    geom_point(alpha = 0.7)

# 单独控制每个图例
p_multi +
    guides(
        color = guide_legend(order = 1, title = "分组"),
        size = guide_legend(order = 2, title = "大小"),
        shape = guide_legend(order = 3, title = "类型")
    ) +
    theme(legend.box = "vertical") # 图例垂直排列
```

---

## 第三部分：标题与注释

### 完整标题系统

```{r}
p_base +
    labs(
        title = "这是主标题",
        subtitle = "这是副标题，提供额外说明",
        caption = "数据来源：模拟数据 | 作者：XXX",
        tag = "A", # 图形标签（多图时有用）
        x = "X 变量",
        y = "Y 变量"
    )
```

### 标题样式定制

```{r}
p_base +
    labs(
        title = "发动机性能与油耗关系",
        subtitle = "基于 200 个观测值的分析",
        caption = "数据来源：模拟数据"
    ) +
    theme(
        # 主标题
        plot.title = element_text(
            size = 18,
            face = "bold",
            color = "#1f2937",
            hjust = 0.5, # 居中
            margin = margin(b = 10)
        ),
        # 副标题
        plot.subtitle = element_text(
            size = 12,
            color = "#6b7280",
            hjust = 0.5,
            margin = margin(b = 15)
        ),
        # 脚注
        plot.caption = element_text(
            size = 9,
            color = "#9ca3af",
            hjust = 1, # 右对齐
            face = "italic"
        ),
        # 图形标签
        plot.tag = element_text(
            size = 16,
            face = "bold"
        ),
        plot.tag.position = c(0.02, 0.98) # 标签位置
    )
```

### 添加文本注释

```{r}
p_base +
    # annotate 添加单个注释
    annotate(
        "text",
        x = 25, y = 280,
        label = "高值区域",
        size = 5,
        color = "#ef4444",
        fontface = "bold"
    ) +
    annotate(
        "text",
        x = 10, y = 80,
        label = "低值区域",
        size = 5,
        color = "#10b981",
        fontface = "bold"
    )
```

### 添加数学公式

```{r}
p_base +
    annotate(
        "text",
        x = 20, y = 300,
        label = "y == beta[0] + beta[1]*x + epsilon",
        parse = TRUE, # 解析为表达式
        size = 5
    )
```

### 添加矩形/线条注释

```{r}
p_base +
    # 矩形高亮区域
    annotate(
        "rect",
        xmin = 18, xmax = 28,
        ymin = 180, ymax = 320,
        alpha = 0.2,
        fill = "#4f46e5"
    ) +
    # 垂直参考线
    geom_vline(xintercept = 20, linetype = "dashed", color = "#ef4444", linewidth = 1) +
    # 水平参考线
    geom_hline(yintercept = 200, linetype = "dotted", color = "#10b981", linewidth = 1) +
    # 箭头注释
    annotate(
        "segment",
        x = 28, xend = 25,
        y = 100, yend = 180,
        arrow = arrow(length = unit(0.3, "cm")),
        color = "#4f46e5"
    ) +
    annotate("text", x = 28, y = 90, label = "注意这个区域", size = 4)
```

### 带标签的数据点

```{r}
# 只标注特定点
highlight_data <- demo_data %>%
    filter(y > 280 | y < 70)

p_base +
    geom_text(
        data = highlight_data,
        aes(label = round(y, 0)),
        vjust = -1,
        size = 3
    )
```

```{r}
# 使用 ggrepel 避免标签重叠
library(ggrepel)

p_base +
    geom_text_repel(
        data = highlight_data,
        aes(label = paste0("y=", round(y, 0))),
        size = 3,
        max.overlaps = 20
    )
```

---

## 第四部分：美学映射深入

### 颜色映射

#### 离散颜色

```{r}
# 使用预设调色板
p_base + scale_color_brewer(palette = "Set1")
```

```{r}
p_base + scale_color_brewer(palette = "Dark2")
```

```{r}
# 手动指定颜色
p_base +
    scale_color_manual(
        values = c(
            "A组" = "#4f46e5",
            "B组" = "#10b981",
            "C组" = "#f59e0b",
            "D组" = "#ef4444"
        )
    )
```

#### 连续颜色

```{r}
ggplot(demo_data, aes(x = x, y = y, color = size)) +
    geom_point(size = 4) +
    scale_color_gradient(low = "#bfdbfe", high = "#1e40af") # 两色渐变
```

```{r}
ggplot(demo_data, aes(x = x, y = y, color = size)) +
    geom_point(size = 4) +
    scale_color_gradient2(low = "#ef4444", mid = "white", high = "#4f46e5", midpoint = 5) # 三色渐变
```

```{r}
ggplot(demo_data, aes(x = x, y = y, color = size)) +
    geom_point(size = 4) +
    scale_color_viridis_c(option = "D") # viridis 色盲友好
```

### 4.2 填充色映射

```{r}
ggplot(demo_data, aes(x = group, y = y, fill = group)) +
    geom_boxplot(alpha = 0.8) +
    scale_fill_manual(
        values = c("#4f46e5", "#10b981", "#f59e0b", "#ef4444")
    ) +
    theme_minimal()
```

### 大小映射

```{r}
ggplot(demo_data, aes(x = x, y = y, size = size)) +
    geom_point(color = "#4f46e5", alpha = 0.6) +
    scale_size_continuous(
        range = c(1, 10), # 大小范围
        breaks = c(2, 4, 6, 8),
        name = "数值大小"
    )
```

### 形状映射

```{r}
ggplot(demo_data, aes(x = x, y = y, shape = category, color = group)) +
    geom_point(size = 3) +
    scale_shape_manual(
        values = c("类型1" = 16, "类型2" = 17, "类型3" = 15) # 圆、三角、方块
    )
```

> **形状编号参考**：0-14 空心，15-20 实心，21-25 可填充

### 透明度映射

```{r}
ggplot(demo_data, aes(x = x, y = y, alpha = size)) +
    geom_point(size = 4, color = "#4f46e5") +
    scale_alpha_continuous(range = c(0.2, 1))
```

### 线型映射

```{r}
line_data <- data.frame(
    x = rep(1:10, 3),
    y = c(cumsum(rnorm(10)), cumsum(rnorm(10)) + 10, cumsum(rnorm(10)) + 20),
    type = rep(c("A", "B", "C"), each = 10)
)

ggplot(line_data, aes(x = x, y = y, linetype = type, color = type)) +
    geom_line(linewidth = 1) +
    scale_linetype_manual(values = c("solid", "dashed", "dotted"))
```

---

## 第五部分：分面详细设置

### facet_wrap 单变量分面

```{r}
p_base +
    facet_wrap(~group, ncol = 2) # 指定列数
```

```{r}
p_base +
    facet_wrap(~group, nrow = 1) # 单行排列
```

```{r}
# 自由坐标轴
p_base +
    facet_wrap(~group, scales = "free") # 每个面板独立坐标
```

```{r}
p_base +
    facet_wrap(~group, scales = "free_y") # 只有 Y 轴自由
```

### facet_grid 双变量分面

```{r}
ggplot(demo_data, aes(x = x, y = y, color = group)) +
    geom_point() +
    facet_grid(category ~ group) # 行 ~ 列
```

```{r}
# 行方向分面
ggplot(demo_data, aes(x = x, y = y)) +
    geom_point(color = "#4f46e5") +
    facet_grid(group ~ .) # 按行分面
```

```{r}
# 列方向分面
ggplot(demo_data, aes(x = x, y = y)) +
    geom_point(color = "#4f46e5") +
    facet_grid(. ~ group) # 按列分面
```

### 分面标签样式

```{r}
p_base +
    facet_wrap(~group) +
    theme(
        # 分面标签（strip）
        strip.text = element_text(
            size = 12,
            face = "bold",
            color = "white"
        ),
        # 分面标签背景
        strip.background = element_rect(
            fill = "#4f46e5",
            color = "#4f46e5"
        )
    )
```

### 自定义分面标签

```{r}
# 使用 labeller 函数
custom_labels <- c(
    "A组" = "对照组 (Control)",
    "B组" = "低剂量 (Low)",
    "C组" = "中剂量 (Medium)",
    "D组" = "高剂量 (High)"
)

p_base +
    facet_wrap(~group, labeller = labeller(group = custom_labels))
```

### 分面间距

```{r}
p_base +
    facet_wrap(~group) +
    theme(
        panel.spacing.x = unit(1.5, "cm"), # 水平间距
        panel.spacing.y = unit(1, "cm") # 垂直间距
    )
```

---

## 第六部分：主题系统全面定制

### 预设主题一览

```{r}
p <- ggplot(demo_data, aes(x = x, y = y, color = group)) +
    geom_point()

p + theme_gray() + labs(title = "theme_gray() - 默认")
```

```{r}
p + theme_bw() + labs(title = "theme_bw() - 黑白")
```

```{r}
p + theme_minimal() + labs(title = "theme_minimal() - 极简")
```

```{r}
p + theme_classic() + labs(title = "theme_classic() - 经典")
```

```{r}
p + theme_void() + labs(title = "theme_void() - 空白")
```

### theme() 完整参数

```{r}
p_base +
    labs(
        title = "完全自定义主题示例",
        subtitle = "所有元素都可以定制"
    ) +
    theme(
        # ========== 图形整体 ==========
        plot.background = element_rect(fill = "#fafafa", color = NA),
        plot.margin = margin(20, 20, 20, 20),

        # ========== 面板 ==========
        panel.background = element_rect(fill = "white"),
        panel.border = element_rect(color = "#e5e7eb", fill = NA),
        panel.grid.major = element_line(color = "#f3f4f6", linewidth = 0.5),
        panel.grid.minor = element_blank(),

        # ========== 坐标轴 ==========
        axis.title = element_text(size = 12, face = "bold"),
        axis.text = element_text(size = 10, color = "#374151"),
        axis.line = element_blank(),
        axis.ticks = element_line(color = "#e5e7eb"),

        # ========== 图例 ==========
        legend.position = "bottom",
        legend.background = element_rect(fill = "white", color = "#e5e7eb"),
        legend.title = element_text(face = "bold"),
        legend.key = element_rect(fill = "white"),

        # ========== 标题 ==========
        plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
        plot.subtitle = element_text(size = 11, color = "gray50", hjust = 0.5),

        # ========== 分面 ==========
        strip.background = element_rect(fill = "#4f46e5"),
        strip.text = element_text(color = "white", face = "bold")
    )
```

### 创建可复用主题

```{r}
# 定义自己的主题函数
theme_my_style <- function(base_size = 12) {
    theme_minimal(base_size = base_size) +
        theme(
            # 标题
            plot.title = element_text(face = "bold", hjust = 0.5),
            plot.subtitle = element_text(color = "gray50", hjust = 0.5),
            # 面板
            panel.grid.minor = element_blank(),
            panel.grid.major = element_line(color = "#f0f0f0"),
            # 坐标轴
            axis.title = element_text(face = "bold"),
            axis.text = element_text(color = "#333333"),
            # 图例
            legend.position = "bottom",
            legend.background = element_rect(fill = "white", color = "gray90"),
            # 分面
            strip.background = element_rect(fill = "#4f46e5"),
            strip.text = element_text(color = "white", face = "bold")
        )
}

# 使用自定义主题
p_base +
    labs(title = "使用自定义主题", subtitle = "可复用于所有图形") +
    theme_my_style()
```

### 设置全局主题

```{r}
# 设置后续所有图形的默认主题
theme_set(theme_minimal())

# 向默认主题添加元素
theme_update(
    plot.title = element_text(face = "bold", hjust = 0.5),
    legend.position = "bottom"
)
```

---

## 第七部分：实用技巧汇总

### 隐藏特定元素

```{r}
p_base +
    theme(
        axis.title.x = element_blank(), # 隐藏 X 轴标题
        axis.text.x = element_blank(), # 隐藏 X 轴刻度标签
        axis.ticks.x = element_blank(), # 隐藏 X 轴刻度线
        panel.grid.major.x = element_blank() # 隐藏垂直网格线
    )
```

### 坐标轴比例

```{r}
# 固定 XY 比例（1:1）
ggplot(demo_data, aes(x, y)) +
    geom_point() +
    coord_fixed(ratio = 1) # X:Y = 1:1
```

### 百分比坐标

```{r}
percent_data <- data.frame(
    group = c("A", "B", "C"),
    pct = c(0.35, 0.45, 0.20)
)

ggplot(percent_data, aes(x = group, y = pct, fill = group)) +
    geom_col() +
    scale_y_continuous(labels = scales::percent_format()) +
    scale_fill_brewer(palette = "Set2") +
    theme_minimal()
```

### 科学计数法

```{r}
sci_data <- data.frame(
    x = 1:5,
    y = c(1e3, 1e4, 1e5, 1e6, 1e7)
)

ggplot(sci_data, aes(x, y)) +
    geom_point(size = 4) +
    geom_line() +
    scale_y_continuous(labels = scales::scientific_format())
```

### 日期坐标

```{r}
date_data <- data.frame(
    date = seq(as.Date("2024-01-01"), as.Date("2024-12-31"), by = "month"),
    value = cumsum(rnorm(12, 10, 5))
)

ggplot(date_data, aes(date, value)) +
    geom_line(color = "#4f46e5", linewidth = 1) +
    geom_point(color = "#4f46e5", size = 3) +
    scale_x_date(
        date_breaks = "2 months",
        date_labels = "%Y-%m"
    ) +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
```

---

## 速查表

| 类别 | 函数/参数 | 功能 |
|------|-----------|------|
| **坐标轴范围** | `xlim()`, `ylim()` | 设置范围（删除超范围数据） |
| | `coord_cartesian(xlim, ylim)` | 放大视图（不删除数据） |
| | `scale_*_continuous(limits)` | 精细控制 |
| **刻度** | `breaks` | 主刻度位置 |
| | `labels` | 刻度标签 |
| | `minor_breaks` | 次刻度位置 |
| **图例位置** | `legend.position` | "top", "bottom", "left", "right", "none" |
| | `legend.position = c(x, y)` | 精确位置 (0-1) |
| **图例样式** | `guides()` | 控制图例细节 |
| | `guide_legend()` | 离散图例参数 |
| **标题** | `labs(title, subtitle, caption, tag)` | 添加标题 |
| | `plot.title`, `plot.subtitle` | theme中的样式 |
| **注释** | `annotate()` | 添加文字/几何注释 |
| | `geom_text()`, `geom_label()` | 数据点标签 |
| **分面** | `facet_wrap(~ var)` | 单变量分面 |
| | `facet_grid(row ~ col)` | 双变量分面 |
| | `scales = "free"` | 自由坐标轴 |
| **颜色** | `scale_color_manual(values)` | 手动颜色 |
| | `scale_color_brewer(palette)` | 预设调色板 |
| | `scale_color_gradient()` | 连续渐变 |
| **主题** | `theme_minimal()`, `theme_bw()` | 预设主题 |
| | `theme()` | 自定义所有元素 |

---

## 小结

本教程涵盖了 ggplot2 样式设置的所有重要方面：

1. **坐标轴**：范围、刻度、标签、轴线、变换
2. **图例**：位置、标题、标签、布局、多图例控制
3. **标题注释**：标题系统、文本注释、几何注释
4. **美学映射**：颜色、大小、形状、透明度、线型
5. **分面**：wrap/grid、标签样式、间距控制
6. **主题系统**：预设主题、完整定制、可复用主题

> **建议**：保存常用的 theme() 设置为函数，在所有图形中复用，保持风格统一。

## 参考资源

- [ggplot2 官方参考](https://ggplot2.tidyverse.org/reference/)
- [ggplot2 主题系统文档](https://ggplot2.tidyverse.org/reference/theme.html)
- [scales 包](https://scales.r-lib.org/)
- [ggplot2 Cheatsheet](https://github.com/rstudio/cheatsheets/blob/main/data-visualization.pdf)
