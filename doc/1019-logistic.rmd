...
- 结局是二分类或事件是否发生（病例/对照、成功/失败）。
- 需要同时控制多种混杂因素并给出效应量解释。
- 关注 OR 与置信区间，强调可解释性而非黑箱预测。
...
Logistic 回归属于广义线性模型，常见假设包括：

1. **对数几率与自变量线性**：连续变量在 logit 空间需近似线性。
2. **观测独立**：样本之间相互独立，避免配对或重复测量。
3. **无严重共线性**：变量相关性过高会导致估计不稳定。
4. **事件数充足**：每个预测变量需要足够事件数支撑估计稳定。

对连续变量，可用样条或分段方式检验线性假设；
对重复测量数据应考虑 GEE 或混合效应模型替代。
...
对于连续变量，可用限制性立方样条或分段方法检查线性假设。
若发现明显非线性，可考虑分段或使用样条项。
...
报告应包含 OR、95% CI、p 值与样本事件数，并说明参照组设定。
建议在表格中标出关键变量的效应方向与显著性。
...
- **惩罚回归**：Firth Logistic、Lasso/Ridge 用于小样本或高维。
- **非线性关系**：用样条或分段回归建模。
- **稳健标准误**：在轻度违背假设时提升稳健性。
- **分层分析**：亚组效应与交互项结合。
