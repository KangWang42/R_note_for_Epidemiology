...
5. 按 [共同辨析合同](../tutorial-authoring/references/comparison-and-visuals.md) 检查相邻命令、工具表面、平台和输出格式。选择不同路径会改变权限、产物或验证方式时，在步骤前先给选择入口。

## 写作与示例

围绕“任务 → 前置条件 → 路径选择 → 最小步骤 → 预期结果 → 验证 → 常见失败”组织。命令块必须可复制，但路径、引号、shell 和工作目录需与平台一致。解释关键参数改变什么，不逐字翻译每个选项。
