...
- 对内存敏感场景先用 `select()` 减少列，再进行计算。
- 使用 `by =` 参数时注意分组字段类型，避免隐式字符转换。

## 常见问题排查

- **结果顺序变化**：tidytable 使用 data.table 逻辑，可能与 dplyr 稍有不同。
- **列名重复**：join 后出现 `.x/.y`，需要明确重命名规则。
- **因子被转为字符**：检查数据读取阶段的 `stringsAsFactors` 设置。
- **性能不达预期**：确认对象是否仍为 tidytable，可用 `is_tidytable()` 检查。
